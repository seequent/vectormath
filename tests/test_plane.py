...
import unittest
from vectormath import Vector3Array, Matrix3, Plane
import numpy as np
import six
...
class TestVMathPlane(unittest.TestCase):
    ...
    def test_init_fromNormalAndPoint(self):
        ...
        P1 = Plane(v1)
        self.assertTrue(np.array_equal(P1.centroid, Vector3Array()))
        self.assertTrue(np.array_equal(P1.normal, v1))
        v2 = [1, 2, 3]
        P2 = Plane(v2)
        self.assertTrue(np.array_equal(P2.centroid, Vector3Array()))
        self.assertTrue(np.array_equal(P2.normal, Vector3Array(v2).as_unit()))
        P3 = Plane(v1, v2)
        self.assertTrue(np.array_equal(P3.centroid, Vector3Array(v2)))
        self.assertTrue(np.array_equal(P3.normal, v1))
        P4 = Plane(v2, v1)
        self.assertTrue(np.array_equal(P4.centroid, v1))
        self.assertTrue(np.array_equal(P4.normal, Vector3Array(v2).as_unit()))
    ...
    def test_init_from3Points(self):
        ...
        v3 = [0, 3, 0]
        P1 = Plane(v1, v2, v3)
        self.assertTrue(np.array_equal(P1.normal, Vector3Array([0, 0, 1])))
        self.assertTrue(np.array_equal(P1.centroid, Vector3Array([2, 1, 0])))
        v4 = [3, 0, 0]
        v5 = [0, 0, 3]
        P2 = Plane(v3, v5, v4)
        self.assertTrue(np.allclose(P2.normal, Vector3Array([1, 1, 1]).as_unit()))
        self.assertTrue(np.array_equal(P2.centroid, Vector3Array([1, 1, 1])))
        P3 = Plane(v3, v4, v5)
        self.assertTrue(np.allclose(P3.normal, -Vector3Array([1, 1, 1]).as_unit()))
        self.assertTrue(np.array_equal(P3.centroid, Vector3Array([1, 1, 1])))
    ...
    def test_init_fromStrikeAndDip(self):
        ...
        v1 = [1, 2, 3]
        P2 = Plane(0, 0, v1)
        self.assertTrue(np.array_equal(P2.normal, Vector3Array(0, 0, 1)))
        self.assertTrue(np.array_equal(P2.centroid, Vector3Array(v1)))
        P3 = Plane(90, 0)
        self.assertTrue(np.allclose(P3.normal, Vector3Array(0, 0, 1)))
        self.assertTrue(np.array_equal(P3.centroid, Vector3Array()))
        P4 = Plane(0, 90)
        self.assertTrue(np.allclose(P4.normal, Vector3Array(1, 0, 0)))
        P5 = Plane(90, 90)
        self.assertTrue(np.allclose(P5.normal, Vector3Array(0, -1, 0)))
        P6 = Plane(0, 45)
        self.assertTrue(np.allclose(P6.normal, Vector3Array(1, 0, 1).as_unit()))
        P7 = Plane(12.44, 90)
        P8 = Plane(192.44, 90)
